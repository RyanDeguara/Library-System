...
<?php
		...
		$sql = "SELECT * FROM books
			WHERE (`Categories` LIKE '%".$query."%') OR (`Author` LIKE '%".$query."%')" or die(mysql_error());
			...
			$getQuery = "SELECT * FROM books WHERE (`Categories` LIKE '%".$query."%') OR (`Author` LIKE '%".$query."%') LIMIT " .  $initial_page . ',' . $limit; 
		...
		for($page_number = 1; $page_number<= $total_pages; $page_number++) {  
					echo '<a class="hrefs" href = "search2.php?page=' . $page_number .'&id='.$query.'">' . "Page: ". $page_number . ' </a>';  
